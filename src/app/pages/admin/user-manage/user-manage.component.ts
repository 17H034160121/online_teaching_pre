...
export class UserManageComponent implements OnInit {
  ...
  handleSaveAddUser(): void {
    ...
    if (this.currentAddUserTab === 0) {
      const { account, username, role, classId} = this.addUserForm;
  ...
  handleImportUser(): void {
    ...
    if (this.userImportFile.type !== 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') {
   ...
   */
  handleEditUser(): void {
    const { account, username, role, classId} = this.editUserForm;
    ...
    const params = { account, username, role, classId};
